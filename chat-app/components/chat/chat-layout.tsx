  ...
  if (isLoading) {
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-ark-stack" role="status" aria-label="Loading">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-ark-line border-t-ark-blue" aria-hidden="true" />
          <p className="text-ark-body text-muted-foreground">Loading&hellip;</p>
        </div>
      </div>
    )
  }
      ...
      <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
        <SheetTrigger asChild>
          <Button
            variant="ghost"
            size="icon"
            className="absolute left-ark-stack top-ark-stack z-50 md:hidden"
